...
import memcached.ChannelFutureTransformer.toFuture
import memcached.netty.messages.{GetRequest, SetRequest, _}
import memcached.netty.{MemcachedDecoder, MemcachedEncoder}
...
class NettyClient(host: String = "localhost", port: Int = 11211)
  ...
  def set(key: String, bytes: Array[Byte], flags: Int = 0, expiration: Int = 0): Future[StatusResponse] =
    this.write(new SetRequest(key, bytes, flags, expiration)).mapTo[StatusResponse]

  def get(key: String): Future[GetResponse] =
    this.write(new GetRequest(key)).mapTo[GetResponse]

  def delete(key: String): Future[StatusResponse] =
    this.write(new DeleteRequest(key)).mapTo[StatusResponse]
  ...
  def close(): Future[Client] = {
    ...
    if (this.disconnectFuture == null) {
      Promise.successful[Client](this).future
    } else {
